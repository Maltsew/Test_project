...
def download_from_remote(sftp_obj, remote_dir_name, local_dir_name):
    """доступ к файлу получаем также, как это делает встроенная библиотека FTP,
    но, отличие sFTP от FTP в том, что первый имеет
    встроенную защиту и обмен похож на обмен по SSH. SFTP можно поставить
    отдельно, но в библиотеке paramiko он уже реализован"""
